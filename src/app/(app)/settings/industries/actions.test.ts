...
vi.mock("@/services/industries", () => ({
  createIndustry: createMock,
  updateIndustry: updateMock,
  setIndustryStatus: statusMock,
  deleteIndustry: deleteMock,
}));
...
describe("createIndustryAction", () => {
  it("adds an industry and revalidates the screen", async () => {
    const result = await createIndustryAction(IDLE, form({ name: "  SaaS  " }));

    // Trimmed before the seam: " SaaS " and "SaaS" are the same industry, and the
    ...
    expect(revalidateMock).toHaveBeenCalledWith(paths.settings.industries);
  });

  it("rejects an empty name with zod, before the seam", async () => {
    const result = await createIndustryAction(IDLE, form({ name: "   " }));
  ...
  it("⚠️ guards BEFORE validating or writing", async () => {
    ...
    await expect(createIndustryAction(IDLE, form({ name: "" }))).rejects.toThrow("NEXT_REDIRECT");
    expect(createMock).not.toHaveBeenCalled();
  });
  ...
  it("⚠️ lets the redirect THROUGH rather than catching it", async () => {
    ...
    await expect(createIndustryAction(IDLE, form({ name: "SaaS" }))).rejects.toThrow(
      "NEXT_REDIRECT",
    );
  });
  ...
  it("surfaces the database's message verbatim", async () => {
    createMock.mockRejectedValueOnce(
      new Error('duplicate key value violates unique constraint "industries_name_ci"'),
    );

    const result = await createIndustryAction(IDLE, form({ name: "saas" }));

    expect(result).toMatchObject({ status: "error" });
    expect((result as { message: string }).message).toMatch(/industries_name_ci/);
  });
});

describe("renameIndustryAction", () => {
  it("renames through update_industry", async () => {
    const result = await renameIndustryAction(IDLE, form({ id: INDUSTRY, name: "Software" }));
  ...
  it("rejects a non-uuid id before the seam", async () => {
    const result = await renameIndustryAction(IDLE, form({ id: "nope", name: "Software" }));
  ...
  it("rejects an empty new name", async () => {
    const result = await renameIndustryAction(IDLE, form({ id: INDUSTRY, name: " " }));
...
describe("setIndustryStatusAction — the reversible path", () => {
  it("archives, and says so in words that promise a way back", async () => {
    const result = await setIndustryStatusAction(IDLE, form({ id: INDUSTRY, status: "archived" }));
  ...
  it("restores", async () => {
    const result = await setIndustryStatusAction(IDLE, form({ id: INDUSTRY, status: "active" }));
  ...
  it("rejects a status the CHECK constraint would not accept", async () => {
    ...
    const result = await setIndustryStatusAction(IDLE, form({ id: INDUSTRY, status: "deleted" }));
...
describe("deleteIndustryAction — and the refusal it must not soften", () => {
  it("deletes an unused industry", async () => {
    const result = await deleteIndustryAction(IDLE, form({ id: INDUSTRY }));
  ...
  it("⚠️ passes the refusal through WITH ITS COUNT, unrewritten", async () => {
    ...
    // `delete_industry` counts the Clients recorded in the industry and names the
    // number; replacing that with "Cannot delete this industry" would leave an
  ...
  it("⚠️ re-implements NO part of the delete rule", async () => {
    ...
    await deleteIndustryAction(IDLE, form({ id: INDUSTRY }));
  ...
  it("guards before deleting", async () => {
    ...
    await expect(deleteIndustryAction(IDLE, form({ id: INDUSTRY }))).rejects.toThrow(
      "NEXT_REDIRECT",
    );
