...
// ⚠️ `listStaffDirectory` AND `StaffDirectoryEntry` WERE DELETED HERE (D15).
//
// They existed for one caller: `clients.ts` turning a `writer_id` into an email,
// because `writer_id` referenced `auth.users` and `authenticated` cannot read
// that table. That reference is gone — a writer is a row in `public.writers`,
// read through the client select's own embed — so the directory has no consumer.
//
// ⚠️ THE RPC BEHIND IT, `public.list_staff_directory()`, IS DROPPED SEPARATELY
// and deliberately LATER: see supabase/drop-staff-directory.sql. Dropping it in
// the same script that swaps the foreign key would break the running app in the
// window between applying the SQL and deploying this code.
//
// `list_staff()` below is untouched. It is admin-only and carries role,
// `assigned` and `pending` — a different RPC with a different audience, and the
// boundary ADR 0013 draws.
