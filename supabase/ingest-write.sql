-- ArcBase ingest WRITE side — CONSOLIDATED copy-paste for the Supabase SQL editor
-- (Dashboard → SQL Editor → New query → paste → Run). ADDITIVE ONLY — safe: it
-- creates public.uploads + public.ingest_metrics and DROPS/ALTERS nothing that
-- the analytics engineer owns. Same DDL as
-- supabase/migrations/20260716120000_arcbase_ingest_write.sql. See
-- supabase/INGEST-WRITE-APPLY.md.
