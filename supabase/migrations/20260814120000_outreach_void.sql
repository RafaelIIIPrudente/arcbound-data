...
-- 20260810120000_outreach_email_report_link.sql, which is already applied;
-- editing that file in place would make `supabase db push` skip it and leave
-- the hosted database on the old definition — the exact trap that shipped a
-- broken Report Link gate once already (see supabase/REPORT-LINKS-APPLY.md).
--
-- ⚠️ THIS TWIN IS FOR THE REPO'S RECORD, NOT FOR DEPLOYMENT. The operator
-- applies SQL by pasting supabase/outreach-void.sql into the Supabase SQL
-- editor. Do not `db push`.
--
-- WHY: outreach snapshots are immutable (ADR 0012) and until now there was NO
-- undo path anywhere. A snapshot attributed to the wrong Client was permanent
-- from inside ArcBase, and because the Client's own report reads their LATEST
-- snapshot, the mistake was live on a client-facing surface.
