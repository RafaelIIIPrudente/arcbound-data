-- ArcBase Arcbound Services registry — CLI migration twin of
-- supabase/arcbound-services.sql. Held to the SAME executable SQL by
-- supabase/sql-sync.test.ts (headers may differ, statements may not).
--
-- Creates public.services (the registry of what Arcbound sells) and
-- public.client_services (which Clients receive which), plus six admin-only
-- SECURITY DEFINER functions, a seed of the two code-backed Services, and a
-- backfill derived from existing upload history. See ADR 0015.
--
-- ⚠️ VISIBILITY IS DATA, CAPABILITY IS CODE. A `handler` names an ingestion
-- pipeline that exists in code; a NULL handler is a real listed offering with no
-- upload path, not an error. Adding a row here can never make a pipeline exist.
-- The full rationale — the partial unique index, the immutable handler, the
-- missing cascade — is in the paste script and is not repeated here.
--
-- ⚠️ "Service" here means an ARCBOUND OFFERING, not the Service Seam
-- (`src/services/`, the UI↔data boundary in CONTEXT.md).
