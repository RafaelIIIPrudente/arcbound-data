-- ArcBase Staff Roles — CLI migration twin of supabase/staff-roles.sql.
-- ADDITIVE ONLY: creates public.staff_roles + public.is_admin() and seeds the
-- known admin(s). The two files are held to the SAME executable SQL by
...
-- privilege tiers, not data partitions). Absence of a row means 'analyst', the
-- table is own-row-readable only, and it carries no write policies at all. The
-- full security rationale is in the paste script; it is not repeated here.
--
-- NOTE ON APPLYING: the working path in this repo is pasting supabase/staff-roles.sql
-- into the Supabase SQL editor. This twin exists so `supabase db push` builds the
-- same schema, and so the DDL is reviewable in version control alongside it.
-- ============================================================================
