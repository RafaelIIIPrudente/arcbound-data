-- ArcBase POSTS AS SOLE SOURCE (ADR 0010, slice S3) — the CLI migration copy.
--
-- Twin of supabase/posts-sole-source.sql; the SQL in the two files is identical
-- and supabase/sql-sync.test.ts fails if they drift. Timestamped AFTER
-- 20260820120000_posts_read_view.sql, whose view this supersedes, and after
-- 20260819120000_posts_ownership.sql, whose ingest_metrics this supersedes.
--
-- ⚠️ APPLIED BY STAFF THROUGH THE SUPABASE SQL EDITOR USING THE TWIN SCRIPT,
-- NEVER BY `supabase db push`. `public.clients`, `public.linkedin_posts_staging`
-- and the `bi` schema were created outside this repo's migrations, so the CLI's
-- picture of the schema is not the live one. Runbook:
-- supabase/POSTS-SOLE-SOURCE-APPLY.md.
--
-- ⚠️ THIS IS THE POINT OF NO RETURN. Once the staging write stops, `bi.*` goes
-- stale and a code revert no longer restores a correct system — it restores a
-- report that is silently missing every post uploaded since. Every earlier slice
-- in this workstream was one revert deep; this one is not.
--
...
-- unwritten and unread. Retiring them is a separate, separately-confirmed step.
-- ============================================================================
