-- ArcBase POSTS OWNERSHIP (ADR 0010, slice S1) — the CLI migration copy.
--
-- Twin of supabase/posts-ownership.sql; the SQL in the two files is identical and
-- supabase/sql-sync.test.ts fails if they drift. Timestamped AFTER
-- 20260818140000_writers_registry.sql and 20260818150000_drop_staff_directory.sql,
-- both of which were still unapplied when this was written and both of which go in
-- first.
--
-- ⚠️ APPLIED BY STAFF THROUGH THE SUPABASE SQL EDITOR USING THE TWIN SCRIPT, NEVER
-- BY `supabase db push`. public.clients and public.linkedin_posts_staging were
-- created outside this repo's migrations, so the CLI's picture of the schema is
-- not the live one — and this migration adds a foreign key onto public.clients.
-- The runbook is supabase/POSTS-OWNERSHIP-APPLY.md.
--
-- WHAT IT DOES: gives ArcBase an app-owned, typed public.posts whose attribution
-- is a real client_id foreign key stamped at ingest from the operator's own
-- selection, replaces public.ingest_metrics with a DUAL-WRITE version that keeps
-- writing public.linkedin_posts_staging byte-for-byte as before, and adds a
-- one-time historical backfill.
--
-- ⚠️ IT REPOINTS NO READS AND DROPS NOTHING. bi.* keeps serving every screen and
-- public.posts is populated but queried by nothing. Repointing is slice S2.
-- ============================================================================
