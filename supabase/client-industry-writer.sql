-- ArcBase Client INDUSTRY + WRITER — copy-paste for the Supabase SQL editor
-- (Dashboard → SQL Editor → New query → paste → Run). ADDITIVE ONLY — it creates
-- public.industries, adds two NULLABLE columns to public.clients, and creates six
-- SECURITY DEFINER functions. It DROPS or ALTERS nothing that already exists, and
-- it adds, removes and edits NO policy on public.clients. Same DDL as
-- supabase/migrations/20260818120000_client_industry_writer.sql. Shaping and the
-- six settled decisions: docs/decisions/2026-08-18-client-industry-and-writer.md.
--
-- ⚠️ RUN THE VERIFICATION QUERIES AT THE BOTTOM ONE AT A TIME, NOT AS A BLOCK.
-- The Supabase SQL editor renders ONLY the LAST statement's result set, so a check
-- that returned nothing looks exactly like a check you never ran.
--
-- ═══════════════════════════════════════════════════════════════════════════════
-- ⚠️ THE RULE EVERYTHING HERE FOLLOWS FROM: `clients.name` IS AN ATTRIBUTION KEY.
--
-- `bi.linkedin_post_latest` attributes a scraped post to a Client by joining
--
--     c.name = trim(regexp_replace(s.post_name, '\s*•\s*You\s*$', '', 'i'))
--
-- Exact and case-sensitive. Editing a Client's name therefore silently
-- re-attributes — or STRANDS — every post that person has. Fourteen of Eitan
-- Hoenig's posts were lost that way on 2026-08-18 by a scraper defect; the same
-- failure reachable from inside the app would be self-inflicted, with no upload to
-- point at and nothing on any screen to explain it.
--
-- So this script gives a Client its first mutable fields WITHOUT giving anyone a
-- route to `name` or `linkedin_url`. It does that BY CONSTRUCTION, not by policy:
-- there is still NO update policy on public.clients, so no caller can write the
-- table directly, and the one function that can names exactly two columns. RLS
-- gates ROWS, not COLUMNS — a column-restricting policy is the wrong tool here and
-- Postgres makes it awkward on purpose.
-- ═══════════════════════════════════════════════════════════════════════════════
--
-- ⚠️ NOTHING BELOW HAS EVER EXECUTED IN CI. No Postgres runs in this repo's test
-- suite; supabase/client-industry-writer.test.ts asserts on the TEXT of this file.
-- It proves the script still SAYS what it is supposed to say, never that it works.
-- The constraints, the guards and the delete refusal are unverified until applied.
