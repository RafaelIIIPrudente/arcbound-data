-- ArcBase Staff Roles ADMIN — copy-paste for the Supabase SQL editor
-- (Dashboard → SQL Editor → New query → paste → Run). ADDITIVE ONLY — it creates
-- public.list_staff() + public.set_staff_role() and DROPS/ALTERS nothing that
-- already exists. Same DDL as
-- supabase/migrations/20260802140000_staff_roles_admin.sql.
--
-- ⚠️ RUN VERIFICATION QUERIES ONE AT A TIME, NOT AS A BLOCK.
--
-- The Supabase SQL editor renders ONLY THE LAST statement's result set. Pasting
-- several `select`s together silently discards every earlier result, so a check
-- that returned nothing looks identical to one you never saw. During the S2 apply
-- this nearly produced a false confirmation. Run each check in its own query.
--
-- Apply AFTER supabase/staff-roles.sql (needs public.staff_roles and
-- public.is_admin()). Order relative to staff-roles-enforce.sql does not matter.
--
-- What this adds, and why each part is a SECURITY DEFINER function:
--   • list_staff()     — the roster. `auth.users` is NOT readable by the
--     `authenticated` role, and `public.staff_roles` is own-row-readable only, so
--     NEITHER can be listed by an ordinary caller. This function is the only path,
--     and it is admin-gated inside its own body.
--   • set_staff_role() — the write. `public.staff_roles` has NO write policies at
--     all; the definer owner bypasses RLS, which is exactly why the guard has to
--     live in the body rather than in a policy.
--
-- ⚠️ THE LAST-ADMIN INVARIANT LIVES HERE AND NOWHERE ELSE. `set_staff_role`
-- refuses any change that would leave zero admins. Do NOT re-implement that rule
-- in the app to pre-disable a control: two copies of a rule drift, and the copy
-- users see drifts first, silently, into a lie. The screen renders this function's
-- refusal instead of predicting it.
