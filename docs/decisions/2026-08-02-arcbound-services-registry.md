...
- **Gate on `handler`, NEVER on `slug`.** ⚠️ **THE REASON I GAVE FOR THIS WAS
  WRONG.** I wrote that S2 lets an admin edit a slug, so a rename could silently
  strip a tab. It cannot: `update_service(uuid, text, text, int)` changes name,
  description and sort_order only, and no function in
  `supabase/arcbound-services.sql` mutates `slug` — it is set at creation and
  never again, exactly like `handler` (whose immutability the SQL argues for
  explicitly at line 159). **The call still stands on its real grounds:** `handler`
  is the enum the code maps to a pipeline, and gating on a human-chosen display
  key instead of the machine key would be wrong even where it is safe.
...
### D17 — The client tab row IS the services offered, labelled from the registry

**Asked and answered 2026-08-03**, from a screenshot of `/clients/<id>/report`
showing `OVERVIEW | POSTS | LINKEDIN REPORT | OUTREACH`:

> "I want the navbar in this page to be overview and the services offered. So the
> posts should be in the LinkedIn report sub navbar in that page."

The row becomes **Overview + one tab per Service the Client holds**, each labelled
with the Service's **name from the registry**. `Posts` stops being a top-level tab
and becomes a sub-nav item inside the LinkedIn section.

```
BRYAN WISH
OVERVIEW   LINKEDIN GROWTH   OUTREACH SYSTEM
─────────────────────────────────────────────────
                ↑ names from Settings → Services

   REPORT   POSTS          ← sub-nav, this section
            ─────
```

D6's row had **two** tabs for **one** Service, which is why it never quite read as
"the services offered". This fixes that.

⚠️ **SAFE ONLY BECAUSE OF `services_one_per_handler`.** That partial unique index
(`on public.services (handler) where handler is not null`) means at most ONE
Service carries each handler, so a registry-named tab can never point two ways at
the same route. If that index is ever dropped, this decision breaks.

A **no-pipeline Service is still not a tab** (D2/D6) — it stays on Overview under
Services. So the row is "the services offered **that have somewhere to go**", and
Overview remains the only place the full list appears.

Rejected: fixed section labels in code (stable, but the row describes sections
rather than services and a rename changes nothing); registry-name-with-fixed-
fallback as a _policy_ (the same tab labelled two different ways depending on
whether a database read succeeded).

### D18 — `/posts` keeps its URL; the parent tab lights on both pages

**Asked and answered 2026-08-03.** No route move. The sub-nav links `/report` ⇄
`/posts`, and the LinkedIn tab stays lit on both.

⚠️ **AN EXPLICIT PATH SET, NEVER A `startsWith`.** `client-tabs.tsx` carries a
binding ⚠️: `isActive` is an EXACT pathname match, and no href may be a prefix of
another — a prefix comparison would light **Overview** on every client route,
since `/clients/<id>` prefixes all of them. The rule becomes one exact-match set
per tab:

```
LinkedIn ← { /clients/<id>/report, /clients/<id>/posts }
Outreach ← { /clients/<id>/outreach }
Overview ← { /clients/<id> }
```

Rejected: moving to `/clients/<id>/report/posts` (the URL would mirror the nav and
sit beside the existing `/report/print`, and only `client-tabs.tsx` links to the
old path internally — but every bookmark and open tab on `/posts` would 404, for a
cosmetic gain); leaving the top row blank on `/posts` (keeps the exact-match rule
untouched, but standing inside a section with nothing lit reads as "you are
nowhere").

### D19 — Planner's calls for S6 (stated, not asked; vetoable)

- **The unreadable-registry fallback reuses S4's synthetic-Service trick.** With
  `held === null` there are no Service names to label with, so the fallback labels
  come from a code-side `Record<ServiceHandler, string>`. This is NOT the rejected
  third option: it is the same policy degrading, and D14's banner already discloses
  it on screen. S4 ships the identical pattern as `ALL_PIPELINE_SERVICES` in
  `upload-tabs.tsx`. ⚠️ Put the map in `src/lib/service-access.ts` (pure) — do NOT
  import from `upload-tabs.tsx`, which would drag the whole ingest form tree into
  the client page. The near-duplication is deliberate and should be flagged, not
  refactored away in this slice.
- **The sub-nav renders on BOTH `/report` and `/posts`.** Only on `/report` would
  make Posts a dead end.
- **Only LinkedIn gets a sub-nav.** Outreach has one view today. Build the
  component general, apply it once.
- **Order stays fixed in code (LinkedIn first), never `sortOrder`.** A re-sort in
  Settings must not be able to reorder a Client's navigation.
- **Truncate a long Service name in the tab.** The name is admin-typed free text
  and the row is `whitespace-nowrap` + `overflow-x-auto`; one long name should not
  be able to push the row.
- **Leave the page headings alone.** The tab will read "LinkedIn Growth" while the
  page eyebrow reads "LinkedIn report", and that is correct — the tab names the
  ENGAGEMENT, the eyebrow names the DOCUMENT. Both true, different nouns.
...
| 2026-08-03 | **S5 LANDED and planner-verified** at 123 files / 1,813 tests, exit 0. Five mutations re-applied by the planner, all five red, all files restored byte-identical. **The workstream's build is COMPLETE — S1→S5 all green.** Two things came out of the run: my D16 justification for gating on `handler` was factually wrong (slug is NOT editable — corrected above, the call survives on other grounds), and **a new defect: internal diagnostics now print into the client-facing PDF.** Detail in the S5 handoff's revisions log, rows 3–6.                                                                                                                                                                                                                     |
| 2026-08-03 | **S6 OPENED — the workstream is no longer five slices.** Bryan, from a screenshot of `/clients/<id>/report`: _"I want the navbar in this page to be overview and the services offered. So the posts should be in the LinkedIn report sub navbar."_ **D17** (row = Overview + registry-named Service tabs, Posts demoted to a sub-nav) and **D18** (`/posts` keeps its URL, parent tab lights via an explicit path set) asked and answered; **D19** planner's calls stated. S6 also carries the two open defects from S5's verification.                                                                                                                                                                                                                             |
...
| 2026-08-03 | **S6 LANDED and planner-verified** at 124 files / 1,826 tests, exit 0, 31.8 s with no timeouts. Six mutations re-applied by the planner, all six red (two understated by the executer, as on every slice), all files restored byte-identical. **D17, D18, D19 all implemented as decided.** The tab row is now Overview + registry-named Service tabs; Posts is a sub-nav inside the LinkedIn section; the parent tab lights via an explicit exact-match path SET, and a test asserts the COUNT of current tabs so an overlapping set cannot pass. **Both defects S5 left behind are fixed.** Detail in the S6 handoff's revisions log.                                                                                                                             |
| 2026-08-03 | ⚠️ **A NEW CLASS OF DEFECT THE GATE CANNOT SEE, found while verifying S6.** `actions.ts` exported `HANDLER_LABELS` — a plain object — from a `"use server"` file. Next.js forbids it (`A "use server" file can only export async functions, found object`) and `/settings/services` failed **at runtime**, from S2 onward. Lint, `type:check`, the full test suite and `next build` were **all green** across S1–S5 with that export in place; it surfaced only when a human loaded the page. Fixed by moving the constant to `src/app/(app)/settings/services/handler-labels.ts`. **The lesson generalises past this bug: the automated gate does not enforce the `"use server"` export contract, so a green gate is not evidence a Server Actions module loads.** |
| 2026-08-03 | ⚠️ **`supabase/arcbound-services.sql` REPORTED APPLIED by Bryan** ("done"). **NOT independently verified** — the planner has no Supabase tooling (the connector does an OAuth handshake only) and live-browser checks are off by standing instruction. Recorded as reported, not as confirmed. The registry now becomes ABSOLUTE (D13): the fail-open fallback stops, tabs take their REGISTRY names (`LinkedIn Growth` / `Outreach System`, not the code-side `LinkedIn Metrics`), and the "could not be read" banner disappears. Verification queries (a)–(e) ship in the script's §11 and must be run **one at a time**.                                                                                                                                         |
| 2026-08-03 | ⚠️ **THE FIRST THING THAT WILL LOOK LIKE A BUG AND IS NOT: a Client with NO upload history now shows ONLY the Overview tab.** §10's backfill is `insert … select` over `uploads` / `outreach_uploads`, so it can only assign a Service to a Client ArcBase already holds an upload for — the D10 chicken-and-egg trap, now live rather than theoretical. Registered-but-never-uploaded Clients get zero Services, and D13 makes that absolute. **Recovery path exists and is in the UI:** assign on that Client's **Overview** via `ClientServicesCard` → admin-gated (`if (!isAdmin)` renders read-only), calling `setClientServicesAction`. ⚠️ `setClientServices` REPLACES THE WHOLE SET — it is not a delta.                                                    |
| 2026-08-03 | **Planner's own destructive error, recorded because the protocol changes.** Restoring each mutation with `git checkout -- <file>` reverted five files to HEAD and wiped S6's UNCOMMITTED source changes. Fully recovered and proved identical (same diffstat, same blob hash on the one file with a recorded hash, same green gate). **Standing rule from now on: mutation-test uncommitted work by copying the file aside, never with `git checkout --`.** The user's "keep uncommitted, I commit" instruction is precisely what makes `git checkout` unsafe here.                                                                                                                                                                                                 |
