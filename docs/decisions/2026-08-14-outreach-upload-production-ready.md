...
## S6 — clearing the Services stale claims (LANDED, planner-verified)

Gate: **136 files / 2,244 tests — UNCHANGED**, which is the acceptance criterion
for a comment-only slice. `src/lib/service-access.ts` byte-identical
(`7d8fe7cc…f324f`). Planner-verified: **zero non-comment changed lines** across
all six source files, and no remaining match for any stale phrasing except the
comment documenting the rename.

### ⚠️ The planner's list of eight was wrong — the real count was 14

The planner grepped only `not applied`. That misses `has not been applied`,
uppercase `IS NOT APPLIED`, `is unapplied`, and the `LIVE PATH TODAY` phrasing
entirely. The executer ran four passes and read all 34 files mentioning
`arcbound-services`, finding **14 sites across 14 files (6 source, 8 test)**.

Six the planner missed: `service-gate.tsx:67`, `upload/page.tsx:21`,
`upload/page.test.tsx:78`, the print `page.test.tsx:102`,
`client-tabs.test.tsx:142`, and `service-gate.test.tsx:59`.

⚠️ **The brief told them to search rather than trust the list, and that
instruction is the only reason the other six were found.** A brief that had
simply enumerated the sites would have left six false claims in place.

### ⚠️ THE BRIEF CONTAINED AN IMPOSSIBLE CONSTRAINT, AND THE EXECUTER SURFACED IT

The loudest remaining claim lived in a **test title** —

    it("⚠️ THIS IS THE LIVE PATH TODAY — names the ambiguity in words", …)

— which is an **executable line**. The brief forbade touching executable lines
AND forbade renaming tests, while requiring every false claim be removed. Those
three cannot all hold. **Planner's error.**

The executer stopped, wrote the correction into the comment beneath, and proposed
the replacement title rather than silently renaming. It has since been renamed to
exactly their proposal — `⚠️ THE DEGRADED PATH — names the ambiguity in words` —
with a comment recording the change and noting the assertion is untouched.
✅ Test count unchanged confirms only the NAME moved.

⚠️ **A test name is read far more often than the body beneath it**, so a false
claim in a title outranks one in a comment. Any future comment-only brief must
say what to do when the claim is in a test name.

### The correction's register

Every rewrite states **frequency, not validity**: the applied state is the
ordinary path and the `null` branch is the degraded one. ⚠️ **Nothing claims the
registry cannot fail** — applied ≠ infallible, and every fallback branch stays
reachable the moment a read fails. All eight tests of those branches stand
untouched.

The anchor rewrite (`arcbound-services.ts`) also records the **table-name trap**
that caused the original misreading — `arcbound_services` is only the migration
filename — in the file most likely to be consulted next.

### ⚠️ A naming collision worth fixing eventually

Several sites said "S1's SQL", meaning the **Services registry's** S1 (ADR 0015),
not this workstream's S1 (the outreach void). A reader who knows the void slices
would reasonably misread them. Each rewrite now names
`supabase/arcbound-services.sql` explicitly, which removes the ambiguity as a side
effect.
...
| 10  | 2026-08-14 | **S6 (Services stale claims) landed and planner-verified** — counts UNCHANGED at 136 / 2,244, `service-access.ts` byte-identical (`7d8fe7cc…f324f`), **zero non-comment changed lines** across all six source files. ⚠️ **The planner's list of eight was wrong: the real count was 14** — grepping only `not applied` missed `has not been applied`, `IS NOT APPLIED`, `is unapplied` and the `LIVE PATH TODAY` phrasing. The brief's instruction to SEARCH rather than trust the list is the only reason the other six were found. ⚠️ **The brief also contained an impossible constraint** — the loudest claim lived in a TEST TITLE (an executable line) while the brief forbade both touching executable lines and renaming tests. Planner's error; the executer surfaced it instead of silently renaming, and it has since been renamed to their proposed title with the assertion untouched. Lesson recorded: **a test name is read more often than the body beneath it**, so future comment-only briefs must say what to do when the claim is in a name. |
